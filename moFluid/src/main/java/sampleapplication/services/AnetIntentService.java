package sampleapplication.services;
...
import sampleapplication.fragments.HistoryFragment;
import sampleapplication.fragments.TransactionFragment;
import sampleapplication.models.AnetSingleton;
import sampleapplication.models.CreditCardObject;
...
public class AnetIntentService extends IntentService {
    ...
    private void authenticateUser(ResultReceiver receiver, Intent intent) {
        ...
        try {
            final String loginId = intent.getStringExtra(LoginActivity2.LOGIN_ID_TAG);
            final String password = intent.getStringExtra(LoginActivity2.PASSWORD_TAG);
        ...
        try {
          ...
          ;MyDataBaseAdapter dbAdapter = new MyDataBaseAdapter(MainActivity.INSTANCE);
           ...
           /* final String totalAmountString = intent.getStringExtra(TransactionFragment.AMOUNT_TAG);*/
            ...
            final BigDecimal totalAmount = new BigDecimal(totalPrice);
    ...
    private ShippingAddress createShippingAddress(UserProfileItem userProfileItem) {

        ShippingAddress address=ShippingAddress.createShippingAddress();
        address.setAddress(userProfileItem.getShippingAddress().getStreet());
        address.setFirstName(userProfileItem.getShippingAddress().getFirstName());
        address.setLastName(userProfileItem.getShippingAddress().getLastName());
        address.setCity(userProfileItem.getShippingAddress().getCity());
        address.setState(userProfileItem.getShippingAddress().getState());
        address.setCountry(userProfileItem.getShippingAddress().getCountryId());
        address.setPhoneNumber(userProfileItem.getShippingAddress().getContactNumber());
        address.setZipPostalCode(userProfileItem.getShippingAddress().getZipCode());
        return address;
    }
     ...
     * @param userProfileItem
     */
    public Customer createCustomer(UserProfileItem userProfileItem) {
        Customer testCustomer = Customer.createCustomer();
        Address billingAddress = Address.createAddress();
        billingAddress.setZipPostalCode(userProfileItem.getBillingAddress().getZipCode());
        billingAddress.setFirstName(userProfileItem.getFirstname());
        billingAddress.setLastName(userProfileItem.getLastname());
        billingAddress.setAddress(userProfileItem.getBillingAddress().getStreet());
        billingAddress.setCity(userProfileItem.getBillingAddress().getCity());
        billingAddress.setCountry(userProfileItem.getBillingAddress().getCountryId());
        billingAddress.setState(userProfileItem.getBillingAddress().getState());
