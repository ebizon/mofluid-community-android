...
import sampleapplication.fragments.HistoryRetainedFragment;
import sampleapplication.fragments.LogoutRetainedFragment;
import sampleapplication.fragments.TransactionFragment;
import sampleapplication.fragments.TransactionRetainedFragment;
...
public class NavigationActivity extends AnetBaseActivity implements
        ...
        switch (getCurrentFragmentTag()) {
            case TRANSACTION_FRAGMENT_TAG:
                ...
                if (transactionType.equals(AnetIntentService.ACTION_MAKE_TRANSACTION)) {
                    ...
                    transactionFragment.makeTransaction(zipcode);
                }
            case HISTORY_FRAGMENT_TAG:
                HistoryFragment historyFragment = (HistoryFragment) getSupportFragmentManager().
                        findFragmentByTag(HISTORY_FRAGMENT_TAG);
  ...
  /*  private void updateCheckedSelection(String fragmentFromIntent) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        Log.d("comming data from transection result fragmant", String.valueOf(requestCode)+requestCode+data);
        if(requestCode==2)
        {
            setResult(2,data);
            finish();
